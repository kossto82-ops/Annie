"""File-backed implementation of :class:`SemanticMemoryRepository` (Vision §3, §21).

Persists semantic memories to a JSON file keyed by pattern, rehydrated on load
exactly like beliefs — confidence and stability are re-derived from the recorded
evidence, never persisted as an assertion.
"""
...
class JsonSemanticMemoryStore:
    """A semantic memory store backed by a single JSON file."""
    ...
    def _persist(self) -> None:
        payload = json.dumps(
            [_serialise_memory(m) for m in self._by_id.values()],
            indent=2,
        )
