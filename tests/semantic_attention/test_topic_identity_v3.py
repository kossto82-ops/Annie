...
external attention window).  These tests pin the invariants the implementation
must satisfy; they are the acceptance tests of the Topic Identity v3 change.
...
B. Narrowing          -- a generic repeat of a detailed mention joins, never
                         splits ({FAIL,DELIVER,TIME}→{FAIL,DELIVER} is one topic).
C. Broadening         -- the founder's canonical signature survives later growth
                         ({FAIL,DELIVER}→{FAIL,DELIVER,TIME}→{...,COST}).
...
F. No-pollution       -- single-concept overlap ({FAIL,*}) never merges topics.
...
A = "supplier failed to deliver"  # {FAIL, DELIVER}
B = "vendor missed the delivery deadline"  # {FAIL, DELIVER, TIME}
S = "failed delivery cost the deadline"  # {FAIL, DELIVER, TIME, COST}
...
def _family() -> list[EpisodeRecord]:
    return [_external("1", A), _external("2", B), _external("3", A)]
...
class TestAOrderReverse:
    def test_both_directions_form_one_topic(self):
        a = _external("1", A)
        b = _external("2", B)
        assert len(resolve_episodes([a, b])) == 1
        assert len(resolve_episodes([b, a])) == 1

    def test_founder_anchors_the_canonical(self):
        a = _external("1", A)
        b = _external("2", B)
        fwd = resolve_episodes([a, b])[0]
        rev = resolve_episodes([b, a])[0]
        ...
        assert fwd.topic_id == "DELIVER > FAIL"
        assert fwd.canonical_signature == frozenset({"FAIL", "DELIVER"})
        assert rev.topic_id == "DELIVER > FAIL > TIME"
        assert rev.canonical_signature == frozenset({"FAIL", "DELIVER", "TIME"})
...
class TestBNarrowing:
    def test_generic_repeat_joins_the_detailed_topic(self):
        # {FAIL, DELIVER, TIME} then {FAIL, DELIVER}: still one topic.
        topic = resolve_episodes([_external("1", B), _external("2", A)])[0]
        assert topic.canonical_signature == frozenset({"FAIL", "DELIVER", "TIME"})
        assert topic.topic_id == "DELIVER > FAIL > TIME"
        assert len(topic.source_episode_ids) == 2


class TestCBroadening:
    def test_founder_anchor_survives_growth(self):
        # {FAIL, DELIVER} -> {FAIL, DELIVER, TIME} -> {FAIL, DELIVER, TIME, COST}.
        topic = resolve_episodes(
            [_external("1", A), _external("2", B), _external("3", S)]
        )[0]
...
class TestDEviction:
    def test_resolution_scope_never_changes_identity(self):
        ...
        assert len(small) == 1
        fam_big = next(t for t in big if t.topic_id == "DELIVER > FAIL")
    ...
    def test_leaving_the_window_shrinks_signals_only(self):
        ...
        had = next(p for p in priorities if p.topic == "DELIVER > FAIL")
        assert had.episodes_on_topic == 2  # only B ("2") and the third A remain

        evicted_two = [*_family(), *[_external(f"f{i}", f"token {i}") for i in range(49)]]
        priorities = derive_attention_priorities(evicted_two)
        had = next(p for p in priorities if p.topic == "DELIVER > FAIL")
        assert had.episodes_on_topic == 1  # only the last A remains
...
class TestEAmbiguity:
    def test_late_synthesis_starts_its_own_topic(self):
        ...
        topics = resolve_episodes([_external("x", A), _external("y", Y), _external("s", S)])
        assert len(topics) == 3

    def test_early_synthesis_absorbs_its_parts(self):
        # S first: the narrowed parts are absorbed into the founder family.
        topic = resolve_episodes([_external("s", S), _external("x", A), _external("y", Y)])[0]
        assert topic.topic_id == "COST > DELIVER > FAIL > TIME"
        assert topic.canonical_signature == frozenset({"FAIL", "DELIVER", "TIME", "COST"})
        assert topic.external_count == 3


class TestFNoPollution:
    def test_single_concept_overlap_never_merges(self):
        # {FAIL,DELIVER}, {FAIL,COST}, {FAIL,TIME} share only FAIL: 3 topics.
        episodes = [
            _external("1", A),
            _external("2", "the failed plan cost us"),
            _external("3", "the failed plan was delayed"),
        ]
...
class TestIRecency:
    def test_historical_count_is_not_current_salience(self):
        ...
        assert all(p.topic != "DELIVER > FAIL" for p in priorities)

        topics = resolve_episodes(history)
        family = next(t for t in topics if t.topic_id == "DELIVER > FAIL")
    ...
    def test_inside_the_window_the_sharp_topic_ranks(self):
        ...
        assert any(
            p.topic == "DELIVER > FAIL" and p.episodes_on_topic == 20 for p in fresh
        )