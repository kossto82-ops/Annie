...
The current project is around Increment 144 (see `STATUS.md`). The reflective cognitive cycle is
...
receives recent turns but each message is still a fresh model call), `TemporalStability` for hypotheses,
or document depth (ownership, per-file ranking, editing via chat). However, **do not assume this
